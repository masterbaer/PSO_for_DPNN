...
def apply_layerwise_structured_ln_pruning_without_last_layer(model):
    # use model.apply(apply_layerwise_pruning) to prune
...
if __name__ == '__main__':
    ...
    for (l0, l0_name), (l1, l1_name), (l2, l2_name), (l3, l3_name), (l_combined, l_combined_name) in zip(
        ...
        if isinstance(l0, torch.nn.Linear):
            ...
            #
                ...
                l_combined.weight.data[number_of_neurons:2 * number_of_neurons, numer_of_inputs:2*numer_of_inputs] += l1.weight.data
                l_combined.weight.data[2 * number_of_neurons:3 * number_of_neurons,2*numer_of_inputs:3*numer_of_inputs] += l2.weight.data
                l_combined.weight.data[3 * number_of_neurons:4 * number_of_neurons, 3*numer_of_inputs:4*numer_of_inputs] += l3.weight.data
                l_combined.bias.data[0:number_of_neurons] += l0.bias.data
                l_combined.bias.data[number_of_neurons:2 * number_of_neurons] += l1.bias.data
                l_combined.bias.data[2 * number_of_neurons:3 * number_of_neurons] += l2.bias.data
                l_combined.bias.data[3 * number_of_neurons:4 * number_of_neurons] += l3.bias.data
    ...
    # TODO remove the pruned neurons from the big model



    # TODO: Vergleich Parameter Averaging vs. 25% Pruning und Concatenation
