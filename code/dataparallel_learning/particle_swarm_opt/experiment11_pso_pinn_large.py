"""
PSO-PINN does not work well as experiment 5 shows. Perhaps increasing the number of particles changes this.
Here we test PSO-PINN with more particles and with more iterations.
"""
...
from parallel_PSO_PINN import PSO_PINN
...
if __name__ == '__main__':
    ...
    # pso = PSO_PINN(model=model, particles_per_rank=10, inertia_weight=0.90,
    ...
    #                  comm=comm)
    pso = PSO_PINN(model=model, particles_per_rank=10, inertia_weight=0.0,
                   social_weight=0.5, cognitive_weight=0.08, max_iterations=5000, train_loader=train_loader,
                   valid_loader=valid_loader, learning_rate=0.01, device=device, rank=rank, world_size=world_size,
                   comm=comm)

    pso.optimize(output1="experiment11_loss.pt", output2="experiment11_accuracy.pt")
