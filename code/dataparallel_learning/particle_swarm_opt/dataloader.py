...
def get_dataloaders_cifar10_half_training_batch_size(batch_size,
                            num_workers=0,
                            root='../../data',
                            validation_fraction=0.1,
                            train_transforms=None,
    ...
    train_loader = torch.utils.data.DataLoader(
        dataset=train_dataset,
        batch_size=batch_size // 2,  # Half the batch size to evaluate max(min(eval1,eval2)) to avoid overestimation.
        num_workers=num_workers,
        drop_last=True,
        sampler=train_sampler
    )
