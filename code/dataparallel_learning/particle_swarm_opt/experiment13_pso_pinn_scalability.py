...
if __name__ == '__main__':
    ...
    #pso = PSO_PINN(model=model, particles_per_rank=particles_per_rank, inertia_weight=0.90,
    ...
    #                   comm=comm)
    pso = PSO_PINN(model=model, particles_per_rank=particles_per_rank, inertia_weight=0.0,
                   social_weight=0.5, cognitive_weight=0.08, max_iterations=1000, train_loader=train_loader,
                   valid_loader=valid_loader, learning_rate=0.01, device=device, rank=rank, world_size=world_size,
                   comm=comm)
